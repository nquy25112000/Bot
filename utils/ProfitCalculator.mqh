...
#ifndef __PROFIT_CALCULATOR_MQH__
#define __PROFIT_CALCULATOR_MQH__
...
double CalcTP(double avgPrice, double totalVol, int entryIndex) {
  if(totalVol <= 0.0) return EMPTY_VALUE;
  double targetCent;
  if(entryIndex < targetByIndex1)      targetCent = 630;
  else if(entryIndex < targetByIndex2) targetCent = 720;
  else                                  targetCent = 900;
  double deltaP = targetCent / (totalVol * 100.0);
  return avgPrice + deltaP;
}

#endif // __PROFIT_CALCULATOR_MQH__
