...
//| Dùng cho FOREX (không phụ thuộc gap). Mặc định D1 shift=1.       |
//+------------------------------------------------------------------+
...
#define LONG_RANGE_ATR_FACTOR  1.30
#define SHORT_RANGE_ATR_FACTOR 0.70
...
enum CandlePattern {
  ...
  PATTERN_PIN_BULL,          // Hammer
  PATTERN_PIN_BEAR,          // Shooting Star
  PATTERN_LONG_BULL,
  PATTERN_LONG_BEAR,
  // 2-candle
...
double _ATR(const string sym, ENUM_TIMEFRAMES tf, int period, int shift_base)
{
  ...
  if(bars < shift_base + period + 5) return 0.0; // bảo vệ thiếu dữ liệu
  int need = period;
  double sumTR = 0.0;
...
PatternScore AssessCandle(const string sym, ENUM_TIMEFRAMES tf = TF_DEFAULT, int shift = 1)
{
  ...
  CandleData c1,c2;
  if(!_GetCandle(sym,tf,shift,c1)) return best;
  ...
  // Single-candle
  {
    ...
    if(s_doji>0){
      ...
      if(score>best.score){ best.id=PATTERN_DOJI; best.name=PatternName(best.id); best.bias="NONE"; best.score=score; best.candlesUsed=1; }
    }
    ...
    if(s_maru>0){
      ...
      if(score>best.score){ best.id=id; best.name=PatternName(id); best.bias=bias; best.score=score; best.candlesUsed=1; }
    }
    double s_pin_b = _PinBullShape(c1);
    if(s_pin_b>0){
      double score = 100.0 * (0.7*s_pin_b + 0.3*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_PIN_BULL; best.name=PatternName(best.id); best.bias="BUY"; best.score=score; best.candlesUsed=1; }
    }
    double s_pin_s = _PinBearShape(c1);
    if(s_pin_s>0){
      double score = 100.0 * (0.7*s_pin_s + 0.3*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_PIN_BEAR; best.name=PatternName(best.id); best.bias="SELL"; best.score=score; best.candlesUsed=1; }
    }
    if(best.id==PATTERN_NONE){
      if(c1.body_ratio >= 0.60){
        ...
        string bias = (c1.bull ? "BUY":"SELL");
        if(score>best.score){ best.id=id; best.name=PatternName(id); best.bias=bias; best.score=score; best.candlesUsed=1; }
      } else if(c1.body_ratio < 0.60 && c1.body_ratio > DOJI_BODY_MAX){
        double balance = 1.0 - MathAbs(c1.upper_ratio - c1.lower_ratio);
        double score = 100.0 * (0.5*balance + 0.5*(1.0 - c1.body_ratio));
        if(score>best.score){ best.id=PATTERN_SPINNING_TOP; best.name=PatternName(best.id); best.bias="NONE"; best.score=score; best.candlesUsed=1; }
      }
    }
  }
  ...
  if(_GetCandle(sym,tf,shift+1,c2))
  {
    ...
    if(s_inside>0){
      ...
      if(score>best.score){ best.id=PATTERN_INSIDE_BAR; best.name=PatternName(best.id); best.bias="NONE"; best.score=score; best.candlesUsed=2; }
    }
    ...
    if(s_out>0){
      ...
      string bias = (c1.bull ? "BUY" : "SELL");
      if(score>best.score){ best.id=id; best.name=PatternName(id); best.bias=bias; best.score=score; best.candlesUsed=2; }
    }
    double s_eng_b = _EngulfBodyBull(c1,c2);
    if(s_eng_b>0){
      double score = 100.0 * (0.6*s_eng_b + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_ENGULFING_BULL; best.name=PatternName(best.id); best.bias="BUY"; best.score=score; best.candlesUsed=2; }
    }
    double s_eng_s = _EngulfBodyBear(c1,c2);
    if(s_eng_s>0){
      double score = 100.0 * (0.6*s_eng_s + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_ENGULFING_BEAR; best.name=PatternName(best.id); best.bias="SELL"; best.score=score; best.candlesUsed=2; }
    }
    double s_har_b = _HaramiBull(c1,c2);
    if(s_har_b>0){
      double score = 100.0 * (0.6*s_har_b + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_HARAMI_BULL; best.name=PatternName(best.id); best.bias="BUY"; best.score=score; best.candlesUsed=2; }
    }
    double s_har_s = _HaramiBear(c1,c2);
    if(s_har_s>0){
      double score = 100.0 * (0.6*s_har_s + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_HARAMI_BEAR; best.name=PatternName(best.id); best.bias="SELL"; best.score=score; best.candlesUsed=2; }
    }
  }
  ...
  {
    double shape;
    if(_ThreeWhiteSoldiers(sym,tf,shift,shape)){
      double score = 100.0 * (0.6*shape + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_3_WHITE_SOLDIERS; best.name=PatternName(best.id); best.bias="BUY"; best.score=score; best.candlesUsed=3; }
    }
    if(_ThreeBlackCrows(sym,tf,shift,shape)){
      double score = 100.0 * (0.6*shape + 0.4*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_3_BLACK_CROWS; best.name=PatternName(best.id); best.bias="SELL"; best.score=score; best.candlesUsed=3; }
    }
    if(_RisingThreeMethods(sym,tf,shift,shape)){
      double score = 100.0 * (0.7*shape + 0.3*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_RISING_3_METHODS; best.name=PatternName(best.id); best.bias="BUY"; best.score=score; best.candlesUsed=5; }
    }
    if(_FallingThreeMethods(sym,tf,shift,shape)){
      double score = 100.0 * (0.7*shape + 0.3*_RangeVsATR(c1,atr));
      if(score>best.score){ best.id=PATTERN_FALLING_3_METHODS; best.name=PatternName(best.id); best.bias="SELL"; best.score=score; best.candlesUsed=5; }
    }
  }
...
{
  datetime t = iTime(_Symbol, TF_DEFAULT, shift); // open time của nến
  if(tzOffsetHours != 0) t += tzOffsetHours * 3600; // nếu muốn quy về UTC+7
