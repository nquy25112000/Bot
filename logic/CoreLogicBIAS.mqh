...
#property strict
#include <Trade\SymbolInfo.mqh> // nếu cần
...
{
   long v1=iVolume(_Symbol,PERIOD_D1,EVAL_SHIFT); if(v1<=0) return false;
   double o=iOpen(_Symbol,PERIOD_D1,EVAL_SHIFT); double c=iClose(_Symbol,PERIOD_D1,EVAL_SHIFT);
   if(c>=o) return false;
   long sum=0; int cnt=0;
   for(int s=EVAL_SHIFT+1;s<=EVAL_SHIFT+5;s++)
      if(iVolume(_Symbol,PERIOD_D1,s)>0){sum+=iVolume(_Symbol,PERIOD_D1,s);cnt++;}
...
// 4. RSI
bool RSIBull()
{ double buf[]; if(CopyBuffer(rsi_handle,0,EVAL_SHIFT,1,buf)!=1) return false; return buf[0]>55.0; }
bool RSIBear()
{ double buf[]; if(CopyBuffer(rsi_handle,0,EVAL_SHIFT,1,buf)!=1) return false; return buf[0]<45.0; }

// 5. MACD
bool MACDBull()
{
   double m[],s[];
   if(CopyBuffer(macd_handle,0,EVAL_SHIFT,1,m)!=1||CopyBuffer(macd_handle,1,EVAL_SHIFT,1,s)!=1) return false;
   return m[0]>s[0];
}
bool MACDBear()
{
...
// 6. MA50
bool MA50Bull()
{ double buf[]; if(CopyBuffer(ma50_handle,0,EVAL_SHIFT,1,buf)!=1) return false; return iClose(_Symbol,PERIOD_D1,EVAL_SHIFT)>buf[0]; }
bool MA50Bear()
{ double buf[]; if(CopyBuffer(ma50_handle,0,EVAL_SHIFT,1,buf)!=1) return false; return iClose(_Symbol,PERIOD_D1,EVAL_SHIFT)<buf[0]; }
...
{
   double h=iHigh(_Symbol,PERIOD_D1,EVAL_SHIFT),l=iLow(_Symbol,PERIOD_D1,EVAL_SHIFT),c=iClose(_Symbol,PERIOD_D1,EVAL_SHIFT);
